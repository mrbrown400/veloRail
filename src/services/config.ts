...
export const CONFIG = {
  ...
  GOOGLE_ROUTES_ENABLED: true,                    // Use Google for bike+rail routing
  GOOGLE_ROUTES_FALLBACK_TO_OSRM: true,           // Fallback to OSRM if Google fails

  // OpenRouteService API key
  ORS_API_KEY: import.meta.env.VITE_ORS_API_KEY || '',

  // Routing preferences
  DEFAULT_SAFETY_PREFERENCE: 'balanced' as const,
...
export function isORSConfigured(): boolean {
  return Boolean(CONFIG.ORS_API_KEY && CONFIG.ORS_API_KEY.length > 0);
}

export function isGoogleMapsConfigured(): boolean {
  return Boolean(CONFIG.GOOGLE_MAPS_API_KEY && CONFIG.GOOGLE_MAPS_API_KEY.length > 0);
}
...
export function isGoogleRoutesEnabled(): boolean {
  return CONFIG.GOOGLE_ROUTES_ENABLED && isGoogleMapsConfigured();
}
