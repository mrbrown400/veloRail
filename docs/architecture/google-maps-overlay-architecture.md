...
`src/stores/mapOverlayStore.ts` is the single visibility source for map overlays. The map control buttons read the registry order and toggle visibility by overlay id. This keeps current, future, visionary, nationalized, and bicycling layers independent instead of coupling visibility to local component state.

## Sample Overlays

The current layer uses Google Maps `TransitLayer`. The future, visionary, and nationalized layers render the VR-001 proposal dataset through Google Maps polylines and station markers:

- Future: D Line Extension seed proposal.
