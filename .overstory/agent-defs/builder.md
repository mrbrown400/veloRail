...
- **Bash:**
  ...
  - `npm run test` (currently a placeholder until automated tests are added)
  - `npm run lint` (currently TypeScript static checking, not style linting)
  - `npm run typecheck` (type checking via tsc)
  - `npm run build` (production build)
  - `npm run quality` (full configured gate)
...
- **Run quality gates before closing.** Do not report completion unless `npm run quality` passes.
...
- **INCOMPLETE_CLOSE** -- Running `bd close` without first passing `npm run quality` and sending a result mail to your parent.
...
1. Run `npm run quality` -- the configured project gate must pass.
