...
If your assigned Seeds issue or spec references a VeloRail role such as `cartographer`, `transit-data`, `scenario`, `ui-designer`, or `infrastructure`, read the matching `agents/<role>.md` file before editing. These files provide domain constraints and ownership notes, not custom Overstory capabilities.
